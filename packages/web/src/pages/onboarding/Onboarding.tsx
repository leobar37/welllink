...
export function Onboarding() {
  ...
  async function updateStep(stepId: string, data: StepPayload) {
    try {
      const { data: resData, error } = await api.api.onboarding.step[stepId].put({
        completed: true,
        stepData: data
      })
  ...
  async function handleNext(data: StepPayload = {}) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 space-y-8">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        {currentStep.id === "welcome" && <StepWelcome onNext={handleNext} />}
        {currentStep.id === "create_profile" && <StepCreateProfile onNext={handleNext} />}
        {currentStep.id === "upload_avatar" && <StepAvatar onNext={handleNext} />}
        {currentStep.id === "add_social_links" && <StepSocial onNext={handleNext} />}
        {currentStep.id === "customize_theme" && <StepTheme onNext={handleNext} />}
        {currentStep.id === "completion" && <StepCompletion />}
      </div>
    </div>
  )
}
