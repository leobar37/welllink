...
export function TuHistoriaPanel({
  ...
  const { data: metrics } = useQuery({
    ...
    queryFn: async () => {
      if (!profileId) return null;
      const { data, error } = await api.api.stories.profile[profileId].metrics.get({
        $query: { days: "30" },
      });
      if (error) throw error;
  ...
  const sectionMutation = useMutation({
    mutationFn: async (formValues: SectionFormValues) => {
      ...
      const { data, error } = await api.api.stories.profile[profileId].config.put(
        payload,
      );
      if (error) throw error;
      return data;
              ...
              <div className="flex flex-col gap-3 rounded-lg border p-4 md:flex-row md:items-end">
                <div className="flex-1 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Este texto aparece en el botón principal (ej. "Mi historia").
                  </p>
                </div>
                <Button
                  type="button"
            ...
            <div className="space-y-3">
              {stories.length === 0 && (
                <Card className="border-dashed">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Sube tu transformación o la de tus clientes para inspirar confianza.
                    </CardDescription>
                  </CardHeader>
                </Card>
              )}
              {stories.map((story, index) => (
                <StoryRow
                  ...
                  disabled={reorderMutation.isPending || deleteStoryMutation.isPending}
                />
              ))}
            </div>
          </section>
...
function StoryRow({
  ...
  const beforeUrl = getAssetPublicUrl(story.beforeAssetId);
  const afterUrl = getAssetPublicUrl(story.afterAssetId);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {defaultValues ? "Editar historia" : "Nueva historia"}
          </DialogTitle>
          <DialogDescription>
            Completa la información y sube las imágenes del antes y después.
          </DialogDescription>
        </DialogHeader>
        <form
          className="space-y-4"
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={watch("type")}
                onValueChange={(value) => setValue("type", value as "self" | "client")}
                disabled={isSubmitting}
              >
                <SelectTrigger>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <ImageUploadField
              ...
              value={beforeAssetId}
              onChange={(value) => setValue("beforeAssetId", value, { shouldValidate: true })}
              disabled={isSubmitting}
            />
            <ImageUploadField
              label="Imagen del después"
              value={afterAssetId}
              onChange={(value) => setValue("afterAssetId", value, { shouldValidate: true })}
              disabled={isSubmitting}
            />
          </div>
...
function ImageUploadField({ label, value, onChange, disabled }: ImageUploadFieldProps) {
  const [uploading, setUploading] = useState(false);
  const previewUrl = getAssetPublicUrl(value);
