...
-- 1. Drop FK constraint from client table
ALTER TABLE client DROP CONSTRAINT IF EXISTS client_health_survey_id_health_survey_response_id_fk;

-- 2. Drop health_survey_id index from client table (if exists)
DROP INDEX IF EXISTS client_health_survey_id_idx;

-- 3. Drop health_survey_id column from client table (keep column but nullable)
...
-- 4. Drop health_survey_response table (this also removes CASCADE dependencies)
DROP TABLE IF EXISTS health_survey_response CASCADE;

-- 5. Drop orphaned indexes from health_survey_response
...
-- 6. ai_recommendation table: REMOVED in separate cleanup (see 0002 migration)
-- The table was never fully utilized and has been removed from codebase
